 ...
 public:
  ...
  Blowfish(bool decrypt, const std::string& key)
    ...
    for (size_t i = 0; i < subkeys_.size(); ++i) {
      uint32_t k =
        (static_cast<uint32_t>(key[(i * 4 + 0) % key.size()]) << 24 & 0b11111111000000000000000000000000) |
        (static_cast<uint32_t>(key[(i * 4 + 1) % key.size()]) << 16 & 0b00000000111111110000000000000000) |
        (static_cast<uint32_t>(key[(i * 4 + 2) % key.size()]) <<  8 & 0b00000000000000001111111100000000) |
        (static_cast<uint32_t>(key[(i * 4 + 3) % key.size()]) <<  0 & 0b00000000000000000000000011111111) ;
      subkeys_[i] ^= k;
    }
    ...
    if (decrypt) {
      std::reverse(subkeys_.begin(), subkeys_.end());
    }
  }
  ...
  void process(char *p, size_t size) {
    ...
    {
      char *q = detail::write_as_bigendian(xL, p);
      detail::write_as_bigendian(xR, q);
    }
  }

 private:
  uint32_t f(uint32_t xL) {
    uint8_t a = static_cast<uint8_t>((xL & 0b11111111000000000000000000000000) >> 24);
    uint8_t b = static_cast<uint8_t>((xL & 0b00000000111111110000000000000000) >> 16);
    uint8_t c = static_cast<uint8_t>((xL & 0b00000000000000001111111100000000) >>  8);
    uint8_t d = static_cast<uint8_t>((xL & 0b00000000000000000000000011111111) >>  0);
    return ((s1_[a] + s2_[b]) ^ s3_[c]) + s4_[d];
  }

  std::vector<uint32_t> subkeys_;
  std::vector<uint32_t> s1_, s2_, s3_, s4_;
};
...
class cipher_streambuf : public std::streambuf {
 ...
 private:
  int overflow(int ch = traits_type::eof()) override {
    ...
    for (size_t i = 0; p + i < pptr(); ++i) {
      *(pbase() + i) = *(p + i);
    }
    pbump(-static_cast<int>(p - pbase()));
